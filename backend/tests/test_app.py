...
class CastingAgencyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.headers = {'Content-Type': 'application/json',
                        'Authorization': f'Bearer {baseauth_token}'}
...
##########################
    ...
    def test_post_actor_as_director(self):
        ...
        exists = Actor.query.filter_by(
            name='Dana Smith', age=25, gender='Female').one_or_none()
        # confirm db save
        self.assertIsNotNone(exists)
        Actor.query.filter_by(name='Dana Smith', age=25,
                              gender='Female').one().delete()
    ...
    def test_post_with_bad_data(self):
        ...
        exists = Actor.query.filter_by(
            name='', age=251, gender='Female').one_or_none()
    ...
    def test_post_movie_as_executive(self):
        ...
        res = self.client().post('/movies', headers=self.headers,
                                 data=json.dumps({'title': 'gr8 movie', 'release_date': '2020-03-15'}))
        self.assertEqual(res.status_code, 200)
        exists = Movie.query.filter_by(
            title='gr8 movie', release_date='2020-03-15').one_or_none()
        # confirm db save
        self.assertIsNotNone(exists)
        Movie.query.filter_by(
            title='gr8 movie', release_date='2020-03-15').one().delete()
##########################################
# Patch
#     Tests
##########################################
    ...
    def test_patch_as_executive(self):
        ...
        res = self.client().patch(
            f'/movies/{movie_id}', data=updates, headers=self.headers)
        # Need the DB to sign off on this as well
        self.assertEqual(res.status_code, 200)
        self.assertEqual(Movie.query.get(
            movie_id).format().get('title', ''), 'new_title')
    ...
    def test_patch_as_director(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertEqual(Movie.query.get(movie_id).format().get(
            'title', ''), 'new_title_for_director')
...
##########################################
    ...
    def test_delete_actor_as_executive(self):
        ...
        res = self.client().delete(
            f'/actors/{actress_id}', headers=self.headers)
        exists = Actor.query.filter_by(
            name='Hannah Carter', age=23, gender='Female').one_or_none()
        self.assertEqual(res.status_code, 200)
        self.assertIsNone(exists)
